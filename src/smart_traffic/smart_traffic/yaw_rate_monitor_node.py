...
def calculate_category_thresholds(csv_path):
    ...
    delta_angles = np.arctan2(
        np.sin(df['rotation_z'] - df['prev_yaw']),
...
class YawRateMonitorNode(Node):
    ...
    def __init__(self):
        ...
        self.yaw_event_pub = self.create_publisher(
            String,
            'yaw_anomaly_ids',
            10
        )
        ...
        for cat_id, threshold in self.thresholds_dict.items():
            ...
            self.get_logger().info(
                f"Set threshold for {name}: {threshold:.2f} rad/s"
            )
    ...
    def listener_callback(self, msg):
        ...
        for vehicle in msg.vehicles:
            ...
            if vehicle_id in self.last_angles:
                ...
                delta_angle = np.arctan2(
                    np.sin(current_angle - last_angle),
                ...
                if current_threshold < yaw_rate < 20.0:
                    self.anomaly_counts[vehicle_id] = (
                        self.anomaly_counts.get(vehicle_id, 0) + 1
                    )

                    if self.anomaly_counts[vehicle_id] >= 3:
                        yaw_event_ids.add(str(vehicle_id))
                        ...
                        if (current_ros_time - last_time) > 3.0:
                            ...
                            self.get_logger().error(
                                f"🚨 SKIDDING: ID {vehicle_id} ({cat_name}) | "
                                f"Rate: {yaw_rate:.2f} rad/s"
                            )

                            self.last_log_time[vehicle_id] = current_ros_time

                        alert_markers.markers.append(
                            self.create_alert_marker(vehicle, header)
                        )
                else:
        ...
        yaw_msg = String()
        yaw_msg.data = ",".join(sorted(yaw_event_ids))
        self.yaw_event_pub.publish(yaw_msg)
...
def main(args=None):
    ...
    try:
        ...
        empty_msg = String()
        empty_msg.data = ""
        node.yaw_event_pub.publish(empty_msg)
